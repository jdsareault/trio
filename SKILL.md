...
1. `nth_send(channel, member_id, "<status>", confidence="high|medium|low", session_token=TOKEN)` — say what you're doing and pass your confidence via the `confidence` param (**high** / **medium** / **low**). It renders as a color-coded badge. Appending the word to the message text still works, but prefer the param.
