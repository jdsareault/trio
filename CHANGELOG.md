...
- **Activity signal** (`server/nth_activity_hook.py`) — a Claude Code **`PreToolUse` + `UserPromptSubmit`** hook (matcher-less) stamps `sessions.last_seen` on *every* tool call and prompt. Before this, `last_seen` was bumped only by trio MCP calls, so an agent reasoning / running a long `Bash` / driving a sub-agent with no trio chatter read as **idle** mid-turn — the dot only lit from the agent's first trio call. Now any tool activity keeps it **working** for the whole active turn (PreToolUse fires at the *start* of each call, so a multi-minute Bash stays green throughout). Reuses `sessions.last_seen` with no schema change: the stall-watchdog's `_resumed()` already keys revival on that column by fingerprint and documented the intent that tool calls bump it, so this makes code match intent and makes revival detection more responsive (a stalled turn runs no tools, so it can't mask a real stall).
...
- **Install** — `setup.sh` registers all three hooks automatically (idempotent per event/script): the turn hook (`Stop` + `StopFailure`), the activity hook (`PreToolUse` + `UserPromptSubmit`), alongside the watchdog's. Needs an MCP-server restart so new sessions capture the `session_id` and the `last_turn_end` column exists (the hook self-heals the column otherwise).
...
- **Ghost pruning on connect** (`nth_server.py`) — joining now clears out any **dead same-name agent "ghost"** first: a same-name member whose **process is gone** is superseded (its tasks released, locks dropped, sessions revoked, row removed, a `[superseded]` line posted) before the newcomer is added. Liveness keys on the member's own `members.last_seen`, which its Monitor heartbeat refreshes (~10s) while the agent's process is alive — including when it's idle **or** stalled-but-revivable — and which is set the instant a member row is inserted; only a process gone longer than the stale threshold is a ghost. (This is deliberately the *opposite* signal from the stall-watchdog, which needs `sessions.last_seen` to tell "working" from "frozen"; here we want to *spare* frozen-but-alive agents, and it also closes a race where a just-joined member that hasn't minted its session yet could be mistaken for a ghost.) Name matching is case/whitespace-insensitive (so `Dev`/`dev`/`Dev ` are one identity, matching the sigil wake key). Genuinely-live same-name members coexist untouched; humans/operators are never pruned. The teardown is shared with `nth_cull` via a `_purge_member` helper. Tests: `tests/test-name-dedup.py` (15 checks).
...
**LOTC round-2 hardening** (batched review): server now caps total ask payload size, rejects blank sub-answers, and enforces single-select cardinality; the client auto-advance only fires on a first-time answer (no more fighting Back-to-correct) with a deselect-race guard, the disabled Submit explains what's missing with jump-to-unanswered, multi-select cues to click Next, and paging moves keyboard focus + labels option groups. Pure client helpers were extracted to `server/nth_ask_client.js` with a Node test (`tests/test-ask-client.js`) so the render-gate bug class can't ship silently again.

**Batched questionnaires.** `trio_ask` also accepts a `questions` list (up to 20 `{question, options, mode?, header?}` items). The human pages forward/back through them in the dashboard and submits every answer at once — one tool call and one reply instead of N of each, à la Claude Code's AskUserQuestion. The message `choices` payload is now `{target, questions:[…]}` (the single-question form is stored as a one-item list; the client still reads the legacy shape), and the answer `selection` is `{answers:[{picked, custom}, …]}`, one entry per question. The picker uses no native radio/checkbox widgets — options are clickable outlined pills; single-select treats a typed answer as an alternative, multi-select allows several options plus multiple "+ add another" free-text answers, all comma-joined.
