...
| `keepalive` | You've been silent for >55min (one turn below the Anthropic prompt-cache TTL) AND the channel has peer traffic within the last 7h. Fires once per quiet period for every member, including hibernators — the whole point is to tap the parent cache BEFORE it expires so re-engagement stays cheap. Suppressed when no peer has posted in >7h (abandoned channel) so we don't spend indefinitely tapping a dead room. | Make one cheap MCP call — `trio_poll(wait_seconds=0)` is the canonical tap — then resume. Do NOT post to the channel; the cache tap is a local concern. If you were hibernating, stay hibernating. |
