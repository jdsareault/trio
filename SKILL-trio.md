...
| `keepalive` | You've been silent for >55min (one turn below the Anthropic prompt-cache TTL). Fires once per quiet period for every member, including hibernators — the whole point is to tap the parent cache BEFORE it expires so re-engagement stays cheap (~$0.13 tap vs ~$2.25 rewrite on a cold wake). | Make one cheap MCP call — `trio_poll(wait_seconds=0)` is the canonical tap — then resume what you were doing. Do NOT post to the channel; the cache tap is a local concern. If you were hibernating, stay hibernating. |
