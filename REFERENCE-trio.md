...
| `trio_ask` | `(channel, member_id, question, options, target, mode?, session_token?)`. Ask a **human** a multiple-choice question rendered as a clickable picker in the dashboard. `options` = 2–12 strings (deduped, ≤300 chars each). `target` = a human's name / guest stem / member id — **rejected if the target is an agent**. `mode="one"` (radio, default) or `"many"` (checkboxes). Pings the target. The human's answer returns as an ordinary `reply_to` message (plain text) plus a stored `selection` for the dashboard — read it like any other message; there's no special answer format. |
