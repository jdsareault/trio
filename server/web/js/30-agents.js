(() => {
  ...
  async function activity(id) { try { const d = await Trio.api.get(`/api/agents/${encodeURIComponent(id)}/activity`); Trio.ui.modal('Agent activity', `<pre>${esc((d.events || []).slice(0, 20).map(e => e.content || e.message || JSON.stringify(e)).join('\n') || 'No activity')}</pre>`); } catch (e) { Trio.ui.toast(e.message); } }
  ...
  function mount() { init(); }
  function unmount() { if (workspaceListener) Trio.events.removeEventListener('workspace:updated', workspaceListener); }
